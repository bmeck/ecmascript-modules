...
const { debuglog, promisify, TextDecoder } = require('util');
...
translators.set('text/javascript', async (url, isMain, body) => {
  let source;
  if (body) {
    source = textDecoder.decode(body);
  } else {
    source = `${await readFileAsync(new URL(url))}`;
  }
...
const winSepRegEx = /\//g;
translators.set('application/node', async (url, isMain, body) => {
  if (body) {
    // this really confuses require.cache
    console.dir({url, body: new Buffer(body).toString()})
    throw Error('cannot provide body to commonjs');
  }
...
translators.set('builtin', async (url, isMain, body) => {
  if (body) {
    throw Error('builtins have no body');
  }
...
translators.set('application/vnd.node.node', async (url, isMain, body) => {
  if (body) {
    // os compat does not allow loading C++ from memory
    throw Error('cannot provide body to native addon');
  }
...
translators.set('application/json', async (url, isMain, body) => {
