...
class Loader {
  ...
  async resolve(specifier, parentURL) {
    ...
    return this._resolve(specifier, parentURL);
  }
  ...
  async getModuleJob(specifier, parentURL) {
    let resolved = await this.resolve(specifier, parentURL);
    let url, format, body;
    while (true) {
      if (resolved.type === 'builtin') {
        url = `node:${resolved.specifier}`;
        format = 'builtin';
      } else if (resolved.type === 'lazy') {
        url = resolved.url;
        format = resolved.format;
      } else if (resolved.type === 'synthetic') {
        const { url_list } = resolved.response;
        if (url_list && url_list.length) {
          const lastURL = url_list[url_list.length - 1];
          url = lastURL;
          if ('node:' === new URL(lastURL).protocol) {
            format = 'builtin';
          } else {
            resolved = await this.resolve(lastURL, parentURL);
            format = resolved.response.type;
            body = resolved.response.buffer;
          }
        } else {
          url = this.moduleMap.createSyntheticLocation();
          format = resolved.response.type;
          body = resolved.response.buffer;
        }
      }
    ...
    job = new ModuleJob(this, url, loaderInstance, parentURL === undefined, body);
